    ...
    <div className="space-y-8">
      {sections.map((key) => (
        <section key={key}>
          ...
          {content2Keys.has(key) && t(`${key}.content2`) && (
            <p className="text-gray-700 leading-relaxed mb-4">{t(`${key}.content2`)}</p>
          )}
      ...
      {contactSectionKey && (
        <section className="bg-gray-100 p-6 rounded-lg border-l-4 border-emerald-600">
          ...
          {t(`${contactSectionKey}.email`) && (
            <p className="text-gray-700 mb-2">
              <strong>{emailLabel}:</strong>{' '}
              <a
                href={`mailto:${t(`${contactSectionKey}.email`)}`}
          ...
          {t(`${contactSectionKey}.address`) && (
            <p className="text-gray-700 mb-4">
              <strong>{addressLabel}:</strong> {t(`${contactSectionKey}.address`)}
            </p>
          )}
