...
export function getComplianceRewriteLines(outDir = OUT) {
  return findCompliancePaths(outDir).map((rel) => `/${rel} /${rel}/index.html 200`);
}
...
export function writeRedirectsFile(outDir = OUT) {
  const complianceLines = getComplianceRewriteLines(outDir);
  const lines = [
    ...
    '/ /index.html 200',
    '',
    '# Store compliance — 200 on exact path (Google Play rejects trailing-slash redirects)',
    ...complianceLines,
    '',
  ...
  console.log(`post-static-export: wrote out/_redirects (${complianceLines.length} compliance rewrites)`);
}
