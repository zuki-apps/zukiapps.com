...
export function mirrorAasaToWellKnown(outDir = OUT) {
  const src = join(outDir, 'apple-app-site-association');
  if (!existsSync(src)) {
    console.warn('post-static-export: apple-app-site-association missing — skip AASA mirror');
    return;
  }
  const destDir = join(outDir, '.well-known');
  mkdirSync(destDir, { recursive: true });
  copyFileSync(src, join(destDir, 'apple-app-site-association'));
  console.log('post-static-export: mirrored apple-app-site-association → .well-known/');
}
