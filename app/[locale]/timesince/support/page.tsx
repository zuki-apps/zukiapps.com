...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'timeSince.support' });
  ...
  return {
    ...
    robots: 'index, follow',
    alternates: {
      canonical: buildCanonical(locale, '/timesince/support'),
      languages: buildLanguageAlternates('/timesince/support'),
    },
  };
}

export default async function TimeSinceSupportPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: 'timeSince.support' });
  const tHero = await getTranslations({ locale, namespace: 'timeSince.hero' });
    ...
    <>
      ...
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: tCommon('support'), path: '/timesince/support' },
        ]}
      />
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="bg-orange-50 rounded-2xl shadow-xl p-8 md:p-12">
            <div className="flex items-center justify-between mb-8 pb-6 border-b-2 border-orange-600">
              <h1 className="text-4xl font-bold text-orange-600">{brandName}</h1>
              <Link
