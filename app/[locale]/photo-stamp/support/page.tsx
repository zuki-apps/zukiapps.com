...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'photoStamp.support' });
  ...
  return {
    ...
    robots: 'index, follow',
    alternates: {
      canonical: buildCanonical(locale, '/photo-stamp/support'),
      languages: buildLanguageAlternates('/photo-stamp/support'),
    },
  };
}
...
export default async function GeoCalcSupportPage({
  ...
  const t = await getTranslations({ locale, namespace: 'photoStamp.support' });
  const tHero = await getTranslations({ locale, namespace: 'photoStamp.hero' });
    ...
    <>
      ...
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: brandName, path: '/photo-stamp' },
          { name: tCommon('support'), path: '/photo-stamp/support' },
        ]}
      />
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-emerald-50 to-gray-50">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="bg-teal-50 rounded-2xl shadow-xl p-8 md:p-12">
            <div className="flex items-center justify-between mb-8 pb-6 border-b-2 border-teal-600">
              <h1 className="text-4xl font-bold text-teal-700">{brandName}</h1>
              <Link
                href={`/${locale}/photo-stamp`}
                className="px-4 py-2 border-2 border-teal-600 bg-white text-teal-700 rounded-lg hover:bg-teal-600 hover:text-white transition-colors text-sm"
              >
                {tCommon('back')}
              </Link>
            </div>
            ...
            <div className={rtl ? 'text-right' : 'text-left'}>
              ...
              <section className="bg-white rounded-lg p-6 mb-8 border-l-4 border-teal-600">
                <div className="flex items-start gap-4">
                  <Mail className="w-6 h-6 text-teal-700 flex-shrink-0 mt-1" />
                  <div>
                    <h3 className="text-2xl font-bold text-teal-700 mb-3">{t('contact.title')}</h3>
              ...
              <section className="mb-8">
                <h3 className="text-2xl font-bold text-teal-700 mb-4">{t('quickLinks.title')}</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  <Link
                    href={`/${locale}/photo-stamp/privacy`}
                  ...
                  <Link
                    href={`/${locale}/photo-stamp/terms`}
                    className="bg-white rounded-lg p-4 hover:shadow-lg transition-shadow border border-gray-200"
                  >
                    <div className="flex items-start gap-3">
                      <FileText className="w-5 h-5 text-teal-700 flex-shrink-0 mt-1" />
                      <div>
              ...
              <section>
                <h3 className="text-2xl font-bold text-teal-700 mb-6">{t('faq.title')}</h3>
                <div className="space-y-6">
                  {faqItems.map((item, index) => (
                    <div key={index} className="bg-white rounded-lg p-6 border border-gray-200">
                      <div className="flex items-start gap-3">
                        <HelpCircle className="w-5 h-5 text-teal-700 flex-shrink-0 mt-1" />
                        <div>
              ...
              <section className="bg-gray-100 p-6 rounded-lg border-l-4 border-teal-600 mt-8">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-teal-700 mb-3">{t('additionalHelp.title')}</h3>
