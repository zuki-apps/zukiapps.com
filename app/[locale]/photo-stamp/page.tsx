...
import { PHOTO_STAMP_ICON } from '@/lib/appIcons';

export default function PhotoStampPage() {
  const t = useTranslations('photoStamp');
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <section className={`py-12 px-4 relative z-10 ${rtl ? 'text-right' : 'text-center'}`} id="download">
        <div className={`max-w-7xl mx-auto ${rtl ? '' : 'text-center'}`}>
          <AppIconFrame
            ...
            frameClassName="rounded-[22%] overflow-hidden shadow-2xl ring-2 ring-teal-400/40"
          />

          <div className="inline-flex items-center gap-2 bg-teal-950/60 border border-teal-500/40 text-teal-200 px-4 py-2 rounded-full text-sm font-semibold mb-6 backdrop-blur-sm">
            <CheckCircle2 className="w-4 h-4" aria-hidden />
            {t('hero.badge')}
          </div>

          <h1 className="text-4xl md:text-5xl font-extrabold mb-4 bg-gradient-to-r from-teal-400 via-emerald-300 to-teal-500 bg-clip-text text-transparent">
            {t('hero.title')}
          </h1>
          <p className="text-xl text-teal-300/90 mb-2">{t('hero.subtitle')}</p>
          ...
          <ProductPageNav namespace="photoStamp" accent="rose" />
          ...
          <div className="mb-10">
            <StoreDownloadBadges
              ...
              fallbackBorderClass="border-teal-600/30"
              utmContent="photo-stamp"
            />
          </div>

          <div className="flex justify-center gap-4 text-sm flex-wrap mb-4">
            <Link href={`/${locale}/photo-stamp/privacy`} className="text-teal-400 hover:text-teal-300 underline">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/photo-stamp/terms`} className="text-teal-400 hover:text-teal-300 underline">
              {tCommon('termsOfService')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/photo-stamp/support`} className="text-teal-400 hover:text-teal-300 underline">
              {tCommon('support')}
            </Link>
          </div>
        </div>
      ...
      <ProductMarketingSections namespace="photoStamp" slug="photo-stamp" accent="rose" hasSupportPage />

      <section className="py-12 px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 border-2 border-teal-500/30 rounded-xl p-8 backdrop-blur-sm bg-opacity-90">
      ...
      <footer className="twilight-footer-bar text-white py-8 px-4 mt-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-4 mt-4 text-sm flex-wrap">
            <Link href={`/${locale}/photo-stamp/support`} className="text-gray-400 hover:text-teal-400 transition-colors">
              {tCommon('support')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/photo-stamp/privacy`} className="text-gray-400 hover:text-teal-400 transition-colors">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/photo-stamp/terms`} className="text-gray-400 hover:text-teal-400 transition-colors">
              {tCommon('termsOfService')}
            </Link>
          </div>
        </div>
