...
import { TALLY_NUDGE_ICON } from '@/lib/appIcons';

export default function TallyNudgePage() {
  const t = useTranslations('tallyNudge');
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <section className={`py-12 px-4 relative z-10 ${rtl ? 'text-right' : 'text-center'}`} id="download">
        <div className={`max-w-7xl mx-auto ${rtl ? '' : 'text-center'}`}>
          <AppIconFrame
            ...
            frameClassName="rounded-[22%] overflow-hidden shadow-2xl ring-2 ring-emerald-400/40"
          />
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold mb-4 bg-gradient-to-r from-emerald-400 via-emerald-300 to-emerald-500 bg-clip-text text-transparent">
            {t('hero.title')}
          </h1>
          <p className="text-xl text-emerald-300/90 mb-2">{t('hero.subtitle')}</p>
          ...
          <ProductPageNav namespace="tallyNudge" accent="emerald" />
          ...
          <div className="mb-10">
            <StoreDownloadBadges
              ...
              fallbackBorderClass="border-emerald-600/30"
              utmContent="tally-nudge"
            />
          </div>

          <div className="flex justify-center gap-4 text-sm flex-wrap mb-4">
            <Link href={`/${locale}/tally-nudge/privacy`} className="text-emerald-400 hover:text-emerald-300 underline">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/tally-nudge/terms`} className="text-emerald-400 hover:text-emerald-300 underline">
              {tCommon('termsOfService')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/tally-nudge/support`} className="text-emerald-400 hover:text-emerald-300 underline">
              {tCommon('support')}
            </Link>
          </div>
        </div>
      ...
      <ProductMarketingSections namespace="tallyNudge" slug="tally-nudge" accent="emerald" hasSupportPage showPromoMascot={false} />

      <section className="py-12 px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 border-2 border-emerald-500/30 rounded-xl p-8 backdrop-blur-sm bg-opacity-90">
      ...
      <footer className="twilight-footer-bar text-white py-8 px-4 mt-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-4 mt-4 text-sm flex-wrap">
            <Link href={`/${locale}/tally-nudge/support`} className="text-gray-400 hover:text-emerald-400 transition-colors">
              {tCommon('support')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/tally-nudge/privacy`} className="text-gray-400 hover:text-emerald-400 transition-colors">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/tally-nudge/terms`} className="text-gray-400 hover:text-emerald-400 transition-colors">
              {tCommon('termsOfService')}
            </Link>
          </div>
        </div>
