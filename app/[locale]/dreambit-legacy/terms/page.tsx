...
export async function generateMetadata({
  ...
  return {
    ...
    robots: 'index, follow',
    alternates: {
      canonical: buildCanonical(locale, '/dreambit-legacy/terms'),
      languages: buildLanguageAlternates('/dreambit-legacy/terms')
    }
  };
}

export default async function DreambitLegacyTermsPage({
  params
}: {
  params: Promise<{ locale: string }>;
}) {
    ...
    <>
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: tRoot('nav.terms'), path: '/dreambit-legacy/terms' }
        ]}
      />
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 py-8">
          ...
          <div className="card-twilight !mb-0">
            ...
            <p className="mt-6 text-sm text-slate-400">
              <Link href={`/${locale}/dreambit-legacy/privacy`} className="text-violet-300 hover:underline">
                {tRoot('nav.privacy')}
              </Link>
            </p>
          </div>
