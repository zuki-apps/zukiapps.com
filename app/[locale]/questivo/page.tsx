...
import { QUESTIVO_ICON } from '@/lib/appIcons';

export default function QuestivoPage() {
  const t = useTranslations('questivo');
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <section className={`py-12 px-4 relative z-10 ${rtl ? 'text-right' : 'text-center'}`} id="download">
        <div className={`max-w-7xl mx-auto ${rtl ? '' : 'text-center'}`}>
          <AppIconFrame
            ...
            frameClassName="rounded-[22%] overflow-hidden shadow-2xl ring-2 ring-violet-400/40"
          />
          ...
          <ProductPageNav namespace="questivo" accent="violet" />
          ...
          <div className="flex justify-center gap-4 text-sm flex-wrap mb-4">
            <Link href={`/${locale}/questivo/privacy`} className="text-violet-400 hover:text-violet-300 underline">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/questivo/terms`} className="text-violet-400 hover:text-violet-300 underline">
              {tCommon('termsOfService')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/questivo/support`} className="text-violet-400 hover:text-violet-300 underline">
              {tCommon('support')}
            </Link>
          </div>
        </div>
      ...
      <ProductMarketingSections namespace="questivo" slug="questivo" accent="violet" hasSupportPage />
      ...
      <footer className="twilight-footer-bar text-white py-8 px-4 mt-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-4 mt-4 text-sm flex-wrap">
            <Link href={`/${locale}/questivo/support`} className="text-gray-400 hover:text-violet-400 transition-colors">
              {tCommon('support')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/questivo/privacy`} className="text-gray-400 hover:text-violet-400 transition-colors">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/questivo/terms`} className="text-gray-400 hover:text-violet-400 transition-colors">
              {tCommon('termsOfService')}
            </Link>
          </div>
        </div>
