...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'whistleCamera.dsa' });
  
  const title = `${t('title')} - Whistle Camera | Zuki Apps`;
  const description = locale === 'he' 
    ? 'ציות לחוק שירותים דיגיטליים (DSA) - Whistle Camera. מידע על זיהוי סוחר, זכויות צרכן וציות לחוקי האיחוד האירופי.'
    : 'Digital Services Act (DSA) Compliance - Whistle Camera. Information about trader identification, consumer rights, and EU compliance.';
  ...
  return {
    ...
    alternates: {
      canonical: locale === routing.defaultLocale && routing.localePrefix === 'as-needed' 
        ? `${baseUrl}/whistle-camera/dsa-compliance` 
        : `${baseUrl}/${locale}/whistle-camera/dsa-compliance`,
      languages: Object.fromEntries(
        routing.locales.map((loc) => [
          loc,
          loc === routing.defaultLocale && routing.localePrefix === 'as-needed'
            ? `${baseUrl}/whistle-camera/dsa-compliance`
            : `${baseUrl}/${loc}/whistle-camera/dsa-compliance`
        ])
      )
    },
    robots: {
    ...
    <>
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: 'DSA Compliance', path: '/whistle-camera/dsa-compliance' }
        ]}
      />
