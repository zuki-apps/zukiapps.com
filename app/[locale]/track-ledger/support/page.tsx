...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'trackLedger.support' });
  ...
  return {
    ...
    robots: 'index, follow',
    alternates: {
      canonical: buildCanonical(locale, '/track-ledger/support'),
      languages: buildLanguageAlternates('/track-ledger/support'),
    },
  };
}

export default async function TrackLedgerSupportPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: 'trackLedger.support' });
  const tHero = await getTranslations({ locale, namespace: 'trackLedger.hero' });
  ...
  return (
    <>
      <Script
        id="track-ledger-support-faq-ld"
      ...
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: tHero('title'), path: '/track-ledger' },
          { name: tCommon('support'), path: '/track-ledger/support' },
        ]}
      />
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="bg-cyan-50 rounded-2xl shadow-xl p-8 md:p-12">
            <div className="flex items-center justify-between mb-8 pb-6 border-b-2 border-cyan-600">
              <h1 className="text-4xl font-bold text-cyan-600">{tHero('title')}</h1>
              <Link
            ...
            <div className={rtl ? 'text-right' : 'text-left'}>
              ...
              <section className="bg-white rounded-lg p-6 mb-8 border-l-4 border-cyan-600">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-cyan-600 mb-3">{t('contact.title')}</h3>
                    <p className="text-gray-700 mb-4">{t('contact.description')}</p>
                    <a
                      href={`mailto:${t('contact.email')}`}
                      className="inline-flex items-center gap-2 text-cyan-600 hover:opacity-80 font-semibold"
                    >
              ...
              <section className="mb-8">
                <h3 className="text-2xl font-bold text-cyan-600 mb-4">{t('quickLinks.title')}</h3>
              ...
              <section>
                <h3 className="text-2xl font-bold text-cyan-600 mb-6">{t('faq.title')}</h3>
              ...
              <section className="bg-gray-100 p-6 rounded-lg border-l-4 border-cyan-600 mt-8">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-cyan-600 mb-3">{t('additionalHelp.title')}</h3>
