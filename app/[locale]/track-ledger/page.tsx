...
import { ArrowLeft, MapPinned, Share2, Map as MapIcon, Mail, ExternalLink, CheckCircle2 } from 'lucide-react';
      ...
      <div className="min-h-screen relative overflow-hidden">
        ...
        <section className="py-12 px-4 relative z-10">
          <div className="max-w-7xl mx-auto text-center">
            <div className="mb-8">
              <AppIconFrame
                src="/images/track-ledger-icon.png"
                alt={t('hero.title')}
                sizes="(max-width: 768px) 96px, 128px"
                priority
                className="mx-auto mb-6"
                boxClassName="w-24 h-24 md:w-32 md:h-32"
                frameClassName="rounded-2xl overflow-hidden shadow-2xl ring-2 ring-cyan-400/40 bg-gradient-to-br from-cyan-950/90 via-slate-950 to-slate-950"
              />
            </div>
            <div className="inline-flex items-center gap-2 bg-cyan-950/50 border border-cyan-500/30 text-cyan-200 px-4 py-2 rounded-full text-sm font-semibold mb-4 backdrop-blur-sm">
              <CheckCircle2 className="w-4 h-4" />
              {t('hero.badge')}
            </div>
            <h1
              className="text-5xl md:text-6xl font-extrabold mb-6 bg-gradient-to-r from-cyan-400 via-sky-300 to-cyan-500 bg-clip-text text-transparent"
              style={{
            ...
            <div className="flex justify-center gap-4 text-sm flex-wrap">
              ...
              <span className="text-gray-400">|</span>
              <Link
                href={`/${locale}/track-ledger/support`}
                className="text-cyan-400 hover:text-cyan-300 underline transition-colors"
              >
                {tCommon('support')}
              </Link>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-8 shadow-lg hover:shadow-xl hover:border-cyan-500/50 border-2 border-cyan-600/30 transition-all backdrop-blur-sm bg-opacity-90">
                ...
                <h3 className="text-2xl font-bold mb-4 text-white">{t('features.gnss.title')}</h3>
                <p className="text-gray-300 mb-4">{t('features.gnss.description')}</p>
                <ul className="text-sm text-gray-400 space-y-2">
                  {t.raw('features.gnss.items').map((item: string, i: number) => (
                    <li key={i}>• {item}</li>
                  ))}
                </ul>
              </div>
              ...
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-8 shadow-lg hover:shadow-xl hover:border-cyan-500/50 border-2 border-cyan-600/30 transition-all backdrop-blur-sm bg-opacity-90">
                <div className="bg-sky-600/30 w-16 h-16 rounded-full flex items-center justify-center mb-6 border border-sky-500/50">
                  <Share2 className="w-8 h-8 text-sky-400" />
                </div>
                <h3 className="text-2xl font-bold mb-4 text-white">{t('features.export.title')}</h3>
                <p className="text-gray-300 mb-4">{t('features.export.description')}</p>
                <ul className="text-sm text-gray-400 space-y-2">
              ...
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-8 shadow-lg hover:shadow-xl hover:border-cyan-500/50 border-2 border-cyan-600/30 transition-all backdrop-blur-sm bg-opacity-90 md:col-span-2 lg:col-span-1">
                <div className="bg-slate-600/30 w-16 h-16 rounded-full flex items-center justify-center mb-6 border border-slate-500/50">
                  <MapIcon className="w-8 h-8 text-slate-200" />
                </div>
                <h3 className="text-2xl font-bold mb-4 text-white">{t('features.map.title')}</h3>
                <p className="text-gray-300 mb-4">{t('features.map.description')}</p>
                <ul className="text-sm text-gray-400 space-y-2">
                  {t.raw('features.map.items').map((item: string, i: number) => (
                    <li key={i}>• {item}</li>
                  ))}
                </ul>
              </div>
        ...
        <ProductMarketingSections namespace="trackLedger" slug="track-ledger" accent="teal" hasSupportPage={true} />
<section className="py-12 px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 border-2 border-cyan-500/30 rounded-xl p-8 backdrop-blur-sm bg-opacity-90">
        ...
        <section className="py-12 px-4 relative z-10">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-1 gap-6 max-w-xl mx-auto">
              <Link
                href={`/${locale}/track-ledger/support`}
                className={`bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-6 shadow-lg hover:shadow-xl hover:border-cyan-500/50 transition-all flex items-center gap-4 group border-2 border-cyan-600/30 backdrop-blur-sm bg-opacity-90 ${locale === 'he' || locale === 'ar' ? 'flex-row-reverse' : ''}`}
              >
