...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'trackLedger.terms' });

  const title = `${t('title')} - Track Ledger | Zuki Apps`;
  const description =
    'Terms of Use for Track Ledger — GPS/GNSS track logging application. Read the terms and conditions for using the app.';

  return {
    title,
    description,
    alternates: {
      canonical: buildCanonical(locale, '/track-ledger/terms'),
      languages: buildLanguageAlternates('/track-ledger/terms'),
    },
    robots: {
      index: true,
      follow: true,
...
export default async function TrackLedgerTermsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
    ...
    <>
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: tCommon('termsOfService'), path: '/track-ledger/terms' },
        ]}
      />
          ...
          <div className="bg-cyan-50 rounded-2xl shadow-xl p-8 md:p-12">
            ...
            <div className={`${locale === 'he' || locale === 'ar' ? 'text-right' : 'text-left'}`}>
              ...
              <div className="space-y-8">
                ...
                <section>
                  <h3 className="text-2xl font-bold text-cyan-600 mb-4">{t('section19.title')}</h3>
                  <p className="text-gray-700 mb-4">{t('section19.content')}</p>
                  <p className="text-gray-700 mb-2">
                    <strong>{tCommon('email')}:</strong>{' '}
                    <a href={`mailto:${t('section19.email')}`} className="text-cyan-600 hover:underline">
                      {t('section19.email')}
                    </a>
                  </p>
                  <p className="text-gray-700 mb-2">
                  ...
                  <p className="text-gray-700 text-sm">
                    <Link href={`/${locale}/track-ledger/privacy`} className="text-cyan-600 hover:underline">
