...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'paratrooperBlitz.support' });
  ...
  return {
    ...
    robots: 'index, follow',
    alternates: {
      canonical: buildCanonical(locale, '/paratrooper-blitz/support'),
      languages: buildLanguageAlternates('/paratrooper-blitz/support'),
    },
  };
}

export default async function ParatrooperBlitzSupportPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: 'paratrooperBlitz.support' });
  const tHero = await getTranslations({ locale, namespace: 'paratrooperBlitz.hero' });
  ...
  return (
    <>
      <Script
        id="paratrooper-blitz-support-faq-ld"
      ...
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: tHero('title'), path: '/paratrooper-blitz' },
          { name: tCommon('support'), path: '/paratrooper-blitz/support' },
        ]}
      />
            ...
            <div className={rtl ? 'text-right' : 'text-left'}>
              ...
              <section className="bg-white rounded-lg p-6 mb-8 border-l-4 border-orange-600">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-orange-600 mb-3">{t('contact.title')}</h3>
                    <p className="text-gray-700 mb-4">{t('contact.description')}</p>
                    <a
                      href={`mailto:${t('contact.email')}`}
                      className="inline-flex items-center gap-2 text-orange-600 hover:opacity-80 font-semibold"
                    >
              ...
              <section className="mb-8">
                <h3 className="text-2xl font-bold text-orange-600 mb-4">{t('quickLinks.title')}</h3>
              ...
              <section>
                <h3 className="text-2xl font-bold text-orange-600 mb-6">{t('faq.title')}</h3>
              ...
              <section className="bg-gray-100 p-6 rounded-lg border-l-4 border-orange-600 mt-8">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-orange-600 mb-3">{t('additionalHelp.title')}</h3>
