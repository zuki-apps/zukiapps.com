...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'coloringMyPhoto.support' });
  ...
  return {
    ...
    robots: 'index, follow',
    alternates: {
      canonical: buildCanonical(locale, '/coloring-my-photo/support'),
      languages: buildLanguageAlternates('/coloring-my-photo/support'),
    },
  };
}
...
export default async function GeoCalcSupportPage({
  ...
  const t = await getTranslations({ locale, namespace: 'coloringMyPhoto.support' });
  const tHero = await getTranslations({ locale, namespace: 'coloringMyPhoto.hero' });
  ...
  return (
    <>
      <Script
        id="coloring-my-photo-support-faq-ld"
      ...
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: brandName, path: '/coloring-my-photo' },
          { name: tCommon('support'), path: '/coloring-my-photo/support' },
        ]}
      />
      <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-gray-50">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="bg-rose-50 rounded-2xl shadow-xl p-8 md:p-12">
            ...
            <div className={rtl ? 'text-right' : 'text-left'}>
              ...
              <section className="bg-white rounded-lg p-6 mb-8 border-l-4 border-rose-600">
                <div className="flex items-start gap-4">
                  <Mail className="w-6 h-6 text-rose-700 flex-shrink-0 mt-1" />
                  <div>
                    <h3 className="text-2xl font-bold text-rose-700 mb-3">{t('contact.title')}</h3>
              ...
              <section className="mb-8">
                <h3 className="text-2xl font-bold text-rose-700 mb-4">{t('quickLinks.title')}</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  <Link
                    href={`/${locale}/coloring-my-photo/privacy`}
                    className="bg-white rounded-lg p-4 hover:shadow-lg transition-shadow border border-gray-200"
                  >
                    <div className="flex items-start gap-3">
                      <Shield className="w-5 h-5 text-rose-700 flex-shrink-0 mt-1" />
                      <div>
                  ...
                  <Link
                    href={`/${locale}/coloring-my-photo/terms`}
                    className="bg-white rounded-lg p-4 hover:shadow-lg transition-shadow border border-gray-200"
                  >
                    <div className="flex items-start gap-3">
                      <FileText className="w-5 h-5 text-rose-700 flex-shrink-0 mt-1" />
                      <div>
              ...
              <section>
                <h3 className="text-2xl font-bold text-rose-700 mb-6">{t('faq.title')}</h3>
                <div className="space-y-6">
                  {faqItems.map((item, index) => (
                    <div key={index} className="bg-white rounded-lg p-6 border border-gray-200">
                      <div className="flex items-start gap-3">
                        <HelpCircle className="w-5 h-5 text-rose-700 flex-shrink-0 mt-1" />
                        <div>
              ...
              <section className="bg-gray-100 p-6 rounded-lg border-l-4 border-rose-600 mt-8">
                <div className="flex items-start gap-3">
                  <MessageCircle className="w-6 h-6 text-rose-700 flex-shrink-0 mt-1" />
                  <div>
                    <h3 className="text-2xl font-bold text-rose-700 mb-3">{t('additionalHelp.title')}</h3>
