...
import { COLORING_MY_PHOTO_ICON } from '@/lib/appIcons';

export default function ColoringMyPhotoPage() {
  const t = useTranslations('coloringMyPhoto');
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <section className={`py-12 px-4 relative z-10 ${rtl ? 'text-right' : 'text-center'}`} id="download">
        <div className={`max-w-7xl mx-auto ${rtl ? '' : 'text-center'}`}>
          <AppIconFrame
            ...
            frameClassName="rounded-[22%] overflow-hidden shadow-2xl ring-2 ring-rose-400/40"
          />

          <div className="inline-flex items-center gap-2 bg-rose-950/60 border border-rose-500/40 text-rose-200 px-4 py-2 rounded-full text-sm font-semibold mb-6 backdrop-blur-sm">
          ...
          <ProductPageNav namespace="coloringMyPhoto" accent="rose" />
          ...
          <div className="mb-10">
            <StoreDownloadBadges
              ...
              utmContent="coloring-my-photo"
            />
          </div>

          <div className="flex justify-center gap-4 text-sm flex-wrap mb-4">
            <Link href={`/${locale}/coloring-my-photo/privacy`} className="text-rose-400 hover:text-rose-300 underline">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/coloring-my-photo/terms`} className="text-rose-400 hover:text-rose-300 underline">
              {tCommon('termsOfService')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/coloring-my-photo/support`} className="text-rose-400 hover:text-rose-300 underline">
              {tCommon('support')}
            </Link>
          </div>
        </div>
      ...
      <ProductMarketingSections namespace="coloringMyPhoto" slug="coloring-my-photo" accent="rose" hasSupportPage />
      ...
      <footer className="twilight-footer-bar text-white py-8 px-4 mt-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-4 mt-4 text-sm flex-wrap">
            <Link href={`/${locale}/coloring-my-photo/support`} className="text-gray-400 hover:text-rose-400 transition-colors">
              {tCommon('support')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/coloring-my-photo/privacy`} className="text-gray-400 hover:text-rose-400 transition-colors">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/coloring-my-photo/terms`} className="text-gray-400 hover:text-rose-400 transition-colors">
              {tCommon('termsOfService')}
            </Link>
          </div>
        </div>
