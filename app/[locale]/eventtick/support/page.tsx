...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'eventTick.support' });
  ...
  return {
    ...
    robots: 'index, follow',
    alternates: {
      canonical: buildCanonical(locale, '/eventtick/support'),
      languages: buildLanguageAlternates('/eventtick/support'),
    },
  };
}
...
export default async function GeoCalcSupportPage({
  ...
  const t = await getTranslations({ locale, namespace: 'eventTick.support' });
  const tHero = await getTranslations({ locale, namespace: 'eventTick.hero' });
    ...
    <>
      ...
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: brandName, path: '/eventtick' },
          { name: tCommon('support'), path: '/eventtick/support' },
        ]}
      />
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-sky-50 to-gray-50">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="bg-sky-50 rounded-2xl shadow-xl p-8 md:p-12">
            <div className="flex items-center justify-between mb-8 pb-6 border-b-2 border-sky-600">
              <h1 className="text-4xl font-bold text-sky-700">{brandName}</h1>
              <Link
                href={`/${locale}/eventtick`}
            ...
            <div className={rtl ? 'text-right' : 'text-left'}>
              ...
              <section className="bg-white rounded-lg p-6 mb-8 border-l-4 border-sky-600">
                <div className="flex items-start gap-4">
                  <Mail className="w-6 h-6 text-sky-700 flex-shrink-0 mt-1" />
                  <div>
                    <h3 className="text-2xl font-bold text-sky-700 mb-3">{t('contact.title')}</h3>
              ...
              <section className="mb-8">
                <h3 className="text-2xl font-bold text-sky-700 mb-4">{t('quickLinks.title')}</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  <Link
                    href={`/${locale}/eventtick/privacy`}
                    className="bg-white rounded-lg p-4 hover:shadow-lg transition-shadow border border-gray-200"
                  >
                    <div className="flex items-start gap-3">
                      <Shield className="w-5 h-5 text-sky-700 flex-shrink-0 mt-1" />
                      <div>
                  ...
                  <Link
                    href={`/${locale}/eventtick/terms`}
                    className="bg-white rounded-lg p-4 hover:shadow-lg transition-shadow border border-gray-200"
                  >
                    <div className="flex items-start gap-3">
                      <FileText className="w-5 h-5 text-sky-700 flex-shrink-0 mt-1" />
                      <div>
              ...
              <section>
                <h3 className="text-2xl font-bold text-sky-700 mb-6">{t('faq.title')}</h3>
                <div className="space-y-6">
                  {faqItems.map((item, index) => (
                    <div key={index} className="bg-white rounded-lg p-6 border border-gray-200">
                      <div className="flex items-start gap-3">
                        <HelpCircle className="w-5 h-5 text-sky-700 flex-shrink-0 mt-1" />
                        <div>
              ...
              <section className="bg-gray-100 p-6 rounded-lg border-l-4 border-sky-600 mt-8">
                <div className="flex items-start gap-3">
                  <MessageCircle className="w-6 h-6 text-sky-700 flex-shrink-0 mt-1" />
                  <div>
                    <h3 className="text-2xl font-bold text-sky-700 mb-3">{t('additionalHelp.title')}</h3>
