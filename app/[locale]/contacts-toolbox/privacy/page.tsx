...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'contactsToolbox.privacy' });
  const tHero = await getTranslations({ locale, namespace: 'contactsToolbox' });
  ...
  return {
    ...
    alternates: {
      canonical: buildCanonical(locale, '/contacts-toolbox/privacy'),
      languages: buildLanguageAlternates('/contacts-toolbox/privacy'),
    },
  };
}
...
export default async function GeoCalcPrivacyPage({
  ...
  const tApp = await getTranslations({ locale, namespace: 'contactsToolbox.hero' });
    ...
    <>
      <BreadcrumbsStructuredData
        ...
        items={[
          ...
          { name: brandName, path: '/contacts-toolbox' },
          { name: tCommon('privacyPolicy'), path: '/contacts-toolbox/privacy' },
        ]}
      />
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="bg-violet-50 rounded-2xl shadow-xl p-8 md:p-12">
            <div className="flex items-center justify-between mb-8 pb-6 border-b-2 border-violet-600">
              <h1 className="text-4xl font-bold text-violet-800">{brandName}</h1>
              <Link
                href={`/${locale}/contacts-toolbox`}
                className="px-4 py-2 border-2 border-violet-600 bg-white text-violet-800 rounded-lg hover:bg-violet-600 hover:text-white transition-colors text-sm"
              >
                {tCommon('back')}
              </Link>
            </div>
            ...
            <div className={rtl ? 'text-right' : 'text-left'}>
              ...
              <LegalSections
                ...
                contactExtraLinks={[
                  {
                    href: `/${locale}/contacts-toolbox/terms`,
