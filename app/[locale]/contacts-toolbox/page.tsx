...
export default function ContactsToolboxPage() {
  const t = useTranslations('contactsToolbox');
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <section className={`py-12 px-4 relative z-10 ${rtl ? 'text-right' : 'text-center'}`} id="download">
        <div className={`max-w-7xl mx-auto ${rtl ? '' : 'text-center'}`}>
          <div className="inline-flex items-center gap-2 bg-violet-950/60 border border-violet-500/40 text-violet-200 px-4 py-2 rounded-full text-sm font-semibold mb-6 backdrop-blur-sm">
            <CheckCircle2 className="w-4 h-4" aria-hidden />
            {t('hero.badge')}
          </div>

          <h1 className="text-4xl md:text-5xl font-extrabold mb-4 bg-gradient-to-r from-violet-400 via-violet-300 to-violet-500 bg-clip-text text-transparent">
            {t('hero.title')}
          </h1>
          <p className="text-xl text-violet-300/90 mb-2">{t('hero.subtitle')}</p>
          ...
          <ProductPageNav namespace="contactsToolbox" accent="violet" />
          ...
          <div className="mb-10">
            <StoreDownloadBadges
              ...
              utmContent="contacts-toolbox"
            />
          </div>

          <div className="flex justify-center gap-4 text-sm flex-wrap mb-4">
            <Link href={`/${locale}/contacts-toolbox/privacy`} className="text-violet-400 hover:text-violet-300 underline">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/contacts-toolbox/terms`} className="text-violet-400 hover:text-violet-300 underline">
              {tCommon('termsOfService')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/contacts-toolbox/support`} className="text-violet-400 hover:text-violet-300 underline">
              {tCommon('support')}
            </Link>
          </div>
        </div>
      ...
      <ProductMarketingSections namespace="contactsToolbox" slug="contacts-toolbox" accent="violet" hasSupportPage showPromoMascot={false} />
      ...
      <footer className="twilight-footer-bar text-white py-8 px-4 mt-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-4 mt-4 text-sm flex-wrap">
            <Link href={`/${locale}/contacts-toolbox/support`} className="text-gray-400 hover:text-violet-400 transition-colors">
              {tCommon('support')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/contacts-toolbox/privacy`} className="text-gray-400 hover:text-violet-400 transition-colors">
              {tCommon('privacyPolicy')}
            </Link>
            <span className="text-gray-500">|</span>
            <Link href={`/${locale}/contacts-toolbox/terms`} className="text-gray-400 hover:text-violet-400 transition-colors">
              {tCommon('termsOfService')}
            </Link>
          </div>
        </div>
