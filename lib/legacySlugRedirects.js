...
/** Extra Cloudflare Pages `_redirects` lines beyond slug pairs (case aliases, .html, etc.). */
const EXTRA_PAGES_REDIRECT_LINES = [
...
/** Cloudflare Pages `_redirects` lines (locale + default-locale unprefixed). */
function getPagesRedirectLines() {
  /** @type {string[]} */
  const lines = [];
  for (const [from, to] of SLUG_PAIRS) {
  ...
  lines.push(...EXTRA_PAGES_REDIRECT_LINES);
